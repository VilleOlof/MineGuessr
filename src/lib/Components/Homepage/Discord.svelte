...
{#if user}
	{@const pfp_url = GetDiscordAvatarUrl(user.user_id, user.avatar)}

	<div class="flex items-center gap-4 text-3xl">
		<img src={pfp_url} alt="" class="h-12 w-12 rounded-full" />
	...
	<div class="flex items-center">
		...
		>
			<svg
				...
				fill="currentColor"
				class="h-8 w-8 text-mc-text-black"
				><path
				...
				/></svg
			>
			Log in with Discord</SmallButton
		>
	</div>
{/if}
