...
<div class="flex flex-col text-slate-300">
	...
	<div class="flex items-center gap-2">
		<p class="text-3xl">
			Spelkod: <span class="rounded-md px-1 text-white" class:hide_mp_lobby_code={!code_visible}
				>{client.metadata.game_id ?? 'Laddar in kod...'}</span
			>
		</p>

		<button on:click={() => (code_visible = !code_visible)} class="h-8 w-8 transition-colors">
			{#if code_visible}
				<svg
	...
	<div class="actions flex w-full gap-2 text-2xl text-white">
		<button
			on:click={leave}
			class="w-1/2 bg-slate-600 transition-transform hover:scale-95 active:scale-105">Lämna</button
		>
		<button
			on:click={ready}
			class="w-1/2 bg-slate-600 transition-transform hover:scale-95 active:scale-105">Redo</button
		>
	</div>
</div>
